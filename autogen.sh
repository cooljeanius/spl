...
test -n "`which automake`" && test -x "`which automake`" && automake --add-missing --include-deps --copy --force-missing -Wno-override
test -n "`which autoconf`" && test -x "`which autoconf`" && autoconf --force -Wall -Wno-cross
